...
サブエージェントに作業を依頼する際は、**必ず以下の報告ルールをプロンプトに含めてください**：

### 報告ルールテンプレート（依頼時に必ず含める）

```
## 報告ルール（厳守）

作業完了時は、以下のフォーマットで正直に報告してください：
...
- 実施内容: [実際に行った作業]
- 達成できたこと: [具体的に何ができたか]
- 達成できなかったこと: [具体的に何ができなかったか]
- 失敗/未達の理由: [なぜできなかったか]
- 次のアクション提案: [必要であれば]
```

### 禁止事項
- ❌ 作業が完了していないのに「完了しました」と報告する
- ❌ エラーが発生したのに成功として報告する
- ❌ 不確実なのに断定的に報告する
- ❌ 問題を隠蔽して報告する
...
重要: 作業が完了していないのに「完了しました」と報告しないでください。
失敗した場合は正直に報告してください。
```
...
- **サブエージェントへの依頼時は、必ず「サブエージェント報告ルール」のテンプレートを含める**
- サブエージェントの報告が曖昧な場合は、具体的な結果を再確認する
